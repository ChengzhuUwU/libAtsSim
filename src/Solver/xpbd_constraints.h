...
inline float get_additional_delta_xpbd_2_with_damping(
    ...
    const float denom =
        (weight1 * length_squared_vec(gradient1) +
         weight2 * length_squared_vec(gradient2)) *
            (1.0f + gamma_tilde) +
        alpha_tilde;
...
inline float get_additional_delta_x_xpbd_4_with_damping(
    ...
    const float denom =
        (w_0 * length_squared_vec(gradient_0) +
         w_1 * length_squared_vec(gradient_1) +
         w_2 * length_squared_vec(gradient_2) +
         w_3 * length_squared_vec(gradient_3)) *
            (1.0f + gamma_tilde) +
        alpha_tilde;
...
inline void reset_constraints(
    ...
    if (index < num_faces_total) {
        lambda_triangle_stretch_term[index] = 0.f;
        lambda_triangle_shear_term[index] = 0.f;
    }
    if (index < num_bending_edges_total) { lambda_bending[index] = 0.f; }
    if (index < num_tets_total) {
        lambda_tet_neohookean_hydrostatic_term[index] = 0.f;
        lambda_tet_neohookean_distortion_term[index] = 0.f;
    }
    if (index < num_cloth) {
        lambda_cloth_balloon[3 * index + 0] = 0;
        lambda_cloth_balloon[3 * index + 1] = 0;
        lambda_cloth_balloon[3 * index + 2] = 0;
        sa_cloth_volumn[2 * index + 1] = 0;
    }
}
...
inline void solve_bending_DAB_template(
    ...
    Float3 q2 = -(cross_vec(p3, n2) + cross_vec(n1, p3) * d) / l23 - (cross_vec(p4, n1) + cross_vec(n2, p4) * d) / l24;//  # eq. (27)
    Float3 q1 = -q2 - q3 - q4;
    ...
    // # eq. (29)
    float sum_wq = (w1 * length_squared_vec(q1) + w2 * length_squared_vec(q2) + w3 * length_squared_vec(q3) + w4 * length_squared_vec(q4));
    ...
    float dlambda = (sin_theta * (-C - alpha_tilde * orig_lambda) / (sum_wq + alpha_tilde * sin_theta_sqr));
...
inline void solve_bending_DAB_template_v2(
    ...
    const float Denom = (AlphaInv + BetaDt) * (w1 * length_squared_vec(Grad1) +
...
inline float compute_energy_stress_neohookean(
    ...
    float local_energy = tet_volumn * (0.5f * m_first_lame * square_scalar(J - 1.0f) + 0.5f * m_second_lame * square_scalar(tr - 3.0f));
    return local_energy;
}
...
inline float compute_energy_collision_ground(
    const uint vid,
    const Pointer(Float3) input_position,
    const float stiff,
    const float thickness) {

    Float3 vert_pos = input_position[vid];
    const float proj = vert_pos.y;
...
inline void spatial_hashing_query_vv(const uint vid,
    ...
    int ix_min = fn_intCoord(curr_pos.x - curr_vert_distance);
    int ix_max = fn_intCoord(curr_pos.x + curr_vert_distance);
    int iy_min = fn_intCoord(curr_pos.y - curr_vert_distance);
    int iy_max = fn_intCoord(curr_pos.y + curr_vert_distance);
    int iz_min = fn_intCoord(curr_pos.z - curr_vert_distance);
    int iz_max = fn_intCoord(curr_pos.z + curr_vert_distance);
                    ...
                    if (accessed) {
                        ...
                        for (uint i = cell_start; i < cell_start + cell_size; i++) {
                            ...
                            if (neighbor > vid) {
                                ...
                                if (curr_collision_count == max_vv_num - 1) {
                                    vert_VV_num_broad_phase[vid] = max_vv_num - 1;
                                    return;
                                }
                            }
                        }
...
inline void solve_self_collision_vf_per_collision_pair_template(
    ...
    // const Float3 diff = t;
    const Float3 bary = makeFloat3(weight[1], weight[2], weight[3]);
    const Float3 normal = t / distance;
