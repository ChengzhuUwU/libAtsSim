...
struct TriangleMeshData {
    ...
    TriangleMeshData() {}
    TriangleMeshData(const TriangleMeshData &input) : model_positions(input.model_positions),
...
inline void extract_surface_face_and_vert_from_tets(
    ...
    parallel_for(0, tmp_tets.size(), [&](const uint i) {
        ...
        if (i != tmp_tets.size() - 1) {
            ...
            if (next_face[0] == curr_face[0] && next_face[1] == curr_face[1] && next_face[2] == curr_face[2]) list_face_type[i] = 1;
        }
        if (i != 0) {
            Int4 prev_face = tmp_tets[i - 1];
            if (prev_face[0] == curr_face[0] && prev_face[1] == curr_face[1] && prev_face[2] == curr_face[2]) list_face_type[i] = 2;
        }
    });

    uint num_surface_faces = 0;
    for (const auto &value : list_face_type) {
        if (value == 0) num_surface_faces++;
    }
    ...
    single_thread_for(0, num_tets, [&](uint tid) {
        Int4 tet = input_tets[tid];
        if (is_surface_vert[tet[0]] || is_surface_vert[tet[1]] || is_surface_vert[tet[2]] || is_surface_vert[tet[3]]) {
...
inline void extract_edges_from_surface(
    ...
    parallel_for(0, tmp_faces.size(), [&](const uint i) {
        ...
        if (i != tmp_faces.size() - 1) {
            ...
            if (next_face[0] == curr_face[0] && next_face[1] == curr_face[1]) list_edge_type[i] = 1;
        }
        if (i != 0) {
            Int3 prev_face = tmp_faces[i - 1];
            if (prev_face[0] == curr_face[0] && prev_face[1] == curr_face[1]) list_edge_type[i] = 2;
        }
    });
